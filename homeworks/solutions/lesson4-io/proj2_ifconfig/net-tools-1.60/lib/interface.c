...
void ife_print_long(struct interface *ptr)
{
	...
	{
	 char ziv[100], *ptrZiv = ziv;
	 
	 char kobi[100], *ptrKobi = kobi + strlen( hw->print(ptr->hwaddr));
	 int str_length=30;
	 //static int len_ziv=strlen( hw->print(ptr->hwaddr));
	 //strcpy (ziv   ,(hw->print(ptr->hwaddr)),strlen( hw->print(ptr->hwaddr));
	 memcpy (ziv ,( hw->print(ptr->hwaddr)), strlen( hw->print(ptr->hwaddr)));
	 
	 *ptrKobi = NULL;
	 while (*ptrZiv != NULL)
	 	*ptrKobi-- = *ptrZiv++;
	 
	 printf(("Reverse HWaddr %s  \n "),  kobi);
