...
static int arp_getdevhw(char *ifname, struct sockaddr *sa, struct hwtype *hw)
{
...
#ifdef REVERSE_MAC
	{
		...
		for (i=0; i<14; i++)
		{
			array[i]=(char)ifr.ifr_hwaddr.sa_data[i];
			sa->sa_data[i] = ifr.ifr_hwaddr.sa_data[14-i-1];
			printf("array[%d] = %2x \n\r", i, (unsigned)(unsigned char)array[i]);
		}
	}
#else	
...
static int arp_show(char *name)
{
	...
	/* barak: add local variables for reversing the MAC while showing the ARP table */
	char *rev = NULL;
	int i,len;
	...
	for (; fgets(line, sizeof(line), fp);) {
		...
		/* barak: swap reverse the MAC before printing */
		len = strlen(hwa);
		rev = (char *)malloc(len);
		if (rev)
		{
			...
			for(i = 0; i < len; i++)
			{
			   /* hwa is a string, swapping its characters. nibbles swapping not required here .*/
			   rev[i] = hwa[len-i-1];
			}
		}

	    if (opt_e)
		arp_disp_2(hostname[0] == '?' ? ip : hostname, type, flags, rev/*hwa*/, mask, dev);
	    else
