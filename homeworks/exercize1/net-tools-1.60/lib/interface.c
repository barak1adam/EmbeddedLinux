...
/* barak: add a compilation flag for the exercise of reversing the MAC */
#define REVERSE_MAC
#ifdef IFF_PORTSEL
...
/* ifconfig: here I am reversing the MAC as it is fetched from the kernel.
   it will affect not just ifconfig printouts but any usage of ifconig...
   
   The other option is to reverse it for printouts only (ife_print_long). 
   I am doing that option of printouts only in arp.c*/
#ifdef REVERSE_MAC
	{
		char byte, i;
		for (i=0; i<IFHWADDRLEN; i++)
		{
			byte = (char)ifr.ifr_hwaddr.sa_data[IFHWADDRLEN-i-1];
			/* not just swapping the bytes order, but the nibbles in each byte too ! */
			ife->hwaddr[i] = ( (byte & 0x0F)<<4 | (byte & 0xF0)>>4 );
		}
	}
#else	
...
void ife_print_long(struct interface *ptr)
{
    ...
    if (hw->print != NULL && (! (hw_null_address(hw, ptr->hwaddr) &&
		...
		/* barak: I could reverses the MAC here just for printouts. please refer to arp.c:617 */
